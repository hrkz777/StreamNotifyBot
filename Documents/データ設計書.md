...
- 同じ通知設定と通知種別への同じURLの重複登録を検知できるよう、鍵付きハッシュの保存を検討する
...
利用者ごとに異なる API 利用予算と初回同期方針を保持します。

| カラム | 論理型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `id` | UUID | yes | 内部 ID |
| `platform_code` | string | yes | コードで定義されたプラットフォームコード |
| `quota_budget_units` | integer | no | 1期間にアプリケーションが使用してよい推定利用量。単位制でない場合は未設定 |
| `quota_reserve_units` | integer | no | 優先度の高い状態取得用に残す推定利用量 |
| `quota_warning_percent` | integer | yes | 管理画面へ警告を表示する使用率 |
| `initial_sync_max_items` | integer | yes | 初回同期で取得する最大件数 |
| `initial_sync_lookback_days` | integer | yes | 初回同期で遡る最大日数 |
| `page_size` | integer | yes | 1回の API 呼び出しで要求するページサイズ |

`platform_code` は一意とします。予約枠は利用予算以下、警告率は有効な百分率、同期件数とページサイズはプラットフォーム API の上限以内とします。
...
`(platform_code, period_started_at)` を一意とします。この値は外部での API 利用を含まない推定値であり、外部サービスが示す実際の残量として表示しません。

## 5. 削除と保持

- 通知タイミングまたは表示情報が変わった場合は、同じ配信の既存投稿を削除してから新しい投稿を作成する
- 配信終了通知の投稿は、配信終了後 1 週間を基準に削除する
