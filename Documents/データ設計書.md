...
本書は、`やりたいこと.md` のテーブル案を正規化し、要件を実現するために必要なデータモデルを示します。MariaDB 10.5系を対象とし、開発・自動テストではMariaDB 10.5.29を基準にします。各テーブル定義では読みやすさのため論理型を併記し、物理型は2.1の対応規則と各列の個別指定に従います。

データベースアクセスはDoctrine DBAL 4.4系を使用し、Repositoryでパラメーター化したSQLとトランザクション境界を明示します。初期構成ではDoctrine ORMを使用しません。スキーマ変更はDoctrine Migrationsで管理し、本書のMariaDB物理型、制約、索引、ロック規則をフレームワークの既定値より優先します。
...
すべての業務テーブルは`ENGINE=InnoDB`、`DEFAULT CHARACTER SET=utf8mb4`、`COLLATE=utf8mb4_unicode_520_ci`、`ROW_FORMAT=DYNAMIC`で作成します。DB接続時にセッションタイムゾーンを`+00:00`、トランザクション分離レベルを`READ COMMITTED`へ設定し、少なくとも`STRICT_TRANS_TABLES`、`ONLY_FULL_GROUP_BY`、`ERROR_FOR_DIVISION_BY_ZERO`、`NO_ZERO_DATE`、`NO_ZERO_IN_DATE`、`NO_ENGINE_SUBSTITUTION`をSQLモードへ含めます。

| 論理型・用途 | 物理型・規則 |
| --- | --- |
| UUID | `BINARY(16)`。PHPで生成したUUIDv7を標準バイト順で保存 |
| binary token | `BINARY(16)`。暗号学的乱数から生成しUUIDとして解釈しない |
| datetime | UTCの`DATETIME(6)`。`TIMESTAMP`とゼロ日付は使用しない |
| boolean | `TINYINT UNSIGNED`と`CHECK (列 IN (0, 1))` |
| 通常の件数・回数 | `INT UNSIGNED` |
| 累積件数・楽観ロック版 | `BIGINT UNSIGNED` |
| 比率・倍率 | `DECIMAL(10,4)` |
| コード・状態 | `VARCHAR(32)`または`VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin`と`CHECK` |
| 外部ID | `VARCHAR(255) CHARACTER SET ascii COLLATE ascii_bin` |
| 不透明なページカーソル | `VARCHAR(2048) CHARACTER SET ascii COLLATE ascii_bin` |
| 索引対象の名称・表示名 | `VARCHAR(191) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci` |
| URL | `VARCHAR(2048) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin`。URL本体は索引化しない |
| 暗号文 | `BLOB` |
| XChaCha20-Poly1305ノンス | `BINARY(24)` |
| SHA-256・HMAC-SHA-256 | `BINARY(32)` |
| IPアドレス | `INET6` |
| structured data | `LONGTEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_bin`とJSON検証制約 |

MariaDBのネイティブ`UUID`型は10.7以降のため使用しません。UUID文字列との変換、UUIDv7のバージョン・variant検証、CSVでの小文字標準形式への変換はPHP側で行い、DB独自のバイト入れ替えを行いません。UUIDは識別子であり、認証トークンには使用しません。

状態値にはMariaDBの`ENUM`を使用せず、バイナリ照合の`VARCHAR`と名前付き`CHECK`制約を使用します。外部ID、コード、トークン、カーソルは大文字小文字を勝手に同一視しません。人間向け文字列だけにUnicode照合を適用します。

### 2.2 日時と構造化データ

作成・更新日時、外部イベント時刻、受信時刻、処理時刻、期限を分離し、すべて`DATETIME(6)`で保存します。PHPから保存する値もUTCへ変換し、DB側の現在時刻が必要なリースと期限比較では`UTC_TIMESTAMP(6)`を使用します。`created_at`と`updated_at`は原則としてアプリケーションから明示的に設定します。

可変構造データは監査ログの変更要約など補助情報だけに使用し、次の制約を設定します。
...
CHECK (
    ...
    OR (
        ...
        AND JSON_TYPE(column_name) = 'OBJECT'
    )
)
```

MariaDBの`JSON`は`LONGTEXT`の別名であるため、物理型を`LONGTEXT`として明示します。検索条件、状態、外部ID、日時、一意制約対象をJSONへ格納しません。保存前にPHPでUTF-8、重複キー、深さ、要素数、最大サイズを検証し、監査ログの変更要約は64 KiB以下とします。検索が必要になった項目は通常列へ昇格し、必要な場合だけ生成列へ索引を設定します。

### 2.3 共通列、制約、索引

更新される管理対象行には、必要に応じて`created_at DATETIME(6)`、`updated_at DATETIME(6)`、`lock_version BIGINT UNSIGNED NOT NULL DEFAULT 0`を持たせます。通常の管理画面更新は`id`と更新前の`lock_version`を条件にして版を1増加させ、更新件数0件を競合として扱います。

外部キー列には索引を作成します。参照されるマスターデータは原則`ON DELETE RESTRICT`、完全に親へ従属する秘密情報、回復コード、セッションは`ON DELETE CASCADE`とし、循環するカスケード削除を作りません。監査ログから参照される管理者は墓標として保持します。

一意制約はアプリケーションの事前検索に加える最終防衛線とし、一意制約違反`1062`を冪等成功または利用者向け重複エラーへ分類します。制約、索引、外部キーの名前は`pk_`、`uk_`、`ix_`、`fk_`、`ck_`で始め、MariaDBの識別子長上限に収めます。

処理待ち索引は原則`(status, next_attempt_at, processing_lease_until, id)`、配信ポーリングは`(status, next_poll_at, poll_lease_until, id)`とします。期限削除列、`renew_after`、`next_notification_at`、セッション期限、監査ログの日時・操作者・対象には実際の抽出条件に対応する複合索引を作成します。低選択性の状態だけの索引、長いURL、タイトル、JSONの索引は作成せず、URLは`BINARY(32)`のフィンガープリントで検索します。
...
| `subscription_type` | string | yes | アカウント内で購読を識別するプラットフォーム固有のイベント種別 |
...
| `processing_lease_token` | binary token | no | 現在の処理所有者を識別する128ビットランダム値 |
| `processing_lease_until` | datetime | no | 重複更新を防ぐリース期限 |
| `last_error_code` | string | no | 秘密情報を含まないエラー識別子 |

制約:

- `(platform_account_id, subscription_type)` を一意とする
- `processing_lease_token` と `processing_lease_until` は両方を設定するか、両方を `NULL` とする
- `renew_after` と `expires_at` がともに存在する場合、`renew_after <= expires_at` とする

初期購読種別は、YouTubeを`channel.feed`、Twitchを`stream.online`、`stream.offline`、`channel.update`、TwitCastingを`livestart`、`liveend`、`liveschedulecreate`、`livescheduleupdate`、`livescheduledelete`とします。アカウント登録時に各行を`pending`かつ即時実行対象として、配信者・アカウントと同じDBトランザクションで作成します。外部サービスへの購読要求はコミット後の別処理で行います。

購読更新Cronは、`pending`または`active`で`renew_after`へ到達し、未リースまたはリース期限切れの行だけを原子的に取得します。一時失敗では現在の状態を維持して`failure_count`と次回の`renew_after`を更新し、恒久失敗では`error`へ移行して自動取得を止めます。成功・失敗結果は取得時のリーストークンが一致する場合だけ保存し、所有者が変わった古い処理結果は破棄します。
...
| `poll_lease_token` | binary token | no | 現在のポーリング所有者を識別する128ビットランダム値 |
...
| `lock_version` | unsigned integer | yes | 管理画面更新の楽観ロック版 |
...
| `processing_lease_token` | binary token | no | 現在の処理所有者を識別する128ビットランダム値 |
| `processing_lease_until` | datetime | no | 重複送信を防ぐリース期限 |
...
### 4.33 `schema_migrations`

| カラム | 物理型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `version` | `VARCHAR(191) CHARACTER SET ascii COLLATE ascii_bin` | yes | マイグレーション版。主キー |
| `checksum` | `BINARY(32)` | yes | マイグレーション内容のSHA-256 |
| `applied_at` | `DATETIME(6)` | yes | UTCの適用完了日時 |
| `execution_time_ms` | `INT UNSIGNED` | yes | 実行時間 |

適用済みマイグレーションの内容変更をチェックサムで検出して拒否します。失敗したマイグレーションを適用済みとして記録せず、DDLの実行状態を調査してから前進修正またはバックアップ復元を行います。
...
初期索引は次のとおりとし、実データ量と`EXPLAIN`による実行計画に基づいて追加・削除します。

- `platform_accounts(platform_code, external_id)` unique
- `streams(platform_account_id, external_id)` unique
- `streams(status, next_poll_at, poll_lease_until, id)`
- `streams(status, tracking_ends_at, id)`
- `streams(api_data_purge_at, id)`
- `stream_events(deduplication_key)` unique
- `stream_events(status, next_attempt_at, processing_lease_until, id)`
- `webhook_subscriptions(status, renew_after, processing_lease_until, id)`
- `notification_events(event_key)` unique
- `notification_events(stream_id, status)`
- `notification_events(status, next_attempt_at, processing_lease_until, id)`
...
- `notifications(status, next_attempt_at, processing_lease_until, id)`
- `notifications(status, delete_after, id)`
- `notification_routes(streamer_id, notification_setting_id)` unique
- `administrator_sessions(token_hash)` unique
- `administrator_sessions(administrator_id, revoked_at, absolute_expires_at)`
- `administrator_tokens(token_hash)` unique
- `audit_logs(occurred_at, id)`
- `audit_logs(actor_administrator_id, occurred_at, id)`
- `audit_logs(target_type, target_id, occurred_at, id)`

外部キー列には索引を作成します。低選択性の`status`だけの索引、URL・タイトル・JSONなど長い値の索引は作成しません。索引名は64文字以内とし、同じ左端列を持つ重複索引がないことをマイグレーションレビューで確認します。
...
- MariaDB 10.5では`SKIP LOCKED`を使用せず、Cronの重複起動には原子的更新による期限付きリースを用いる
- 専用キューを使用せず、各業務テーブルの処理状態、再試行時刻、リーストークン、リース期限を Cron ジョブの処理待ち情報として使用する
- 一意制約違反を通常の競合として処理し、重複レコードを作らない
- CSVはステージングで全行検証し、管理者の確定後に1ファイル単位のトランザクションで全件反映または全件ロールバックする

### 8.1 Cronリース

Cron実行または取得バッチごとに推測困難な`BINARY(16)`のリーストークンを生成し、期限到達かつ未リースまたは期限切れの行へ、`UPDATE ... ORDER BY ... LIMIT`を使用してトークンと`UTC_TIMESTAMP(6)`基準の期限を原子的に設定します。更新を即座にコミットし、同じトークンを持つ行だけを取得してから外部通信を開始します。

リース更新、成功、失敗記録では`id`とリーストークン一致を更新条件にします。期限切れ後に別処理が取得した行へ古い所有者が更新を試み、更新件数が0件となった場合は外部取得結果を破棄します。外部通信中にトランザクションや行ロックを保持しません。

### 8.2 行ロックと楽観ロック

最後の有効な`owner`の判定、通知状態の切替、同期完了後の差分確定、保持期間変更の確定など短い整合性処理では、明示的なトランザクション内で`SELECT ... FOR UPDATE`を使用します。複数行はUUIDまたは業務キーの一定順でロックします。

管理画面の通常更新は`lock_version`を用い、`id`と更新前の版が一致する場合だけ更新して版を1増加させます。更新件数0件は別の管理者による競合として、現在値の再読込を要求します。

デッドロック`1213`とロック待機タイムアウト`1205`では、トランザクション全体を最大3回まで短いジッター付きで再試行します。一意制約違反はこの再試行に含めません。

## 9. マイグレーション

- スキーマ変更は順序付きマイグレーションとして管理し、`schema_migrations`へ版、チェックサム、適用日時、実行時間を記録する
- Webリクエストから自動適用せず、CLIから明示的に実行する
- `GET_LOCK('stream-notify-bot:migration', 0)`を取得できた1実行だけが適用し、終了時は成否にかかわらず`RELEASE_LOCK()`して接続を破棄する
- 適用前にMariaDBバージョン、SQLモード、文字セット、照合順序、ストレージエンジンを検証する
- DDLは暗黙コミットされるため、トランザクションによる完全ロールバックを前提にしない
- DDLとデータ移行を可能な限り別のマイグレーションへ分割する
- アプリケーションコードと互換性のある追加、移行、読取切替、削除の段階的変更を優先する
- 破壊的変更の前に、バックアップ、復元、前進修正の手順を確認し、downマイグレーションだけを復旧手段にしない
- 大量更新は小さいバッチに分け、進捗を保存して中断後に再開できるようにする
- 初期データとサンプルデータを区別し、実在の秘密情報を含めない
- MariaDB 10.5.29で自動テストまたはDocker環境の検証を行う
