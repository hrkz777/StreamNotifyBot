...
本書は、`やりたいこと.md` のテーブル案を正規化し、要件を実現するために必要なデータモデルを示します。MariaDB 10.5.29を互換性の下限とし、本番環境では保守期間内のMariaDB 10.11 LTS以降を推奨します。各テーブル定義では読みやすさのため論理型を併記し、物理型は2.1の対応規則と各列の個別指定に従います。
...
- MariaDB 10.5.29と推奨LTS版の差異を自動テストまたはDocker環境で検証する
