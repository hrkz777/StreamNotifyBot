...
配信者画面、通知設定画面、運用設定画面では、入力したモックデータをブラウザーのLocal Storageへ保存します。入力内容はサーバーやデータベースへ送信されず、配信者は「モックデータを消去」、通知設定は設定ごとの「削除」から消去できます。運用設定では確定済みの絶対上下限と項目間制約を画面上でも検証します。

通知設定のテスト送信は画面上のシミュレーションであり、Discordへの通信は行いません。Webhook URLを含む入力値は暗号化されないため、実際の秘密情報は入力しないでください。
