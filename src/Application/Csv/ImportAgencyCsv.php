...
final readonly class ImportAgencyCsv
{
    ...
    public function preview(string $csv): array
    {
        ...
        foreach ($this->reader->read($csv, AgencyCsvCodec::HEADERS, AgencyCsvCodec::SCHEMA_VERSION) as $row) {
            ...
            if (!in_array($row['is_independent'], ['0','1'], true)) {
                throw new CsvFormatException('is_independentは0または1で指定してください。');
            }
    ...
    public function execute(string $csv, bool $replace): void
    {
        $this->agencies->replaceFromCsv($this->preview($csv), $replace);
    }
}
